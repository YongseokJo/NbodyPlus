...
void Particle::calculateRegAccelerationSecondOrder(std::vector<Particle*> &particle) {
	...
	REAL rs2 = 0.1;  // scale radius...? need to check

	REAL dt, r, r2, vx;
	REAL a0_reg[Dim], a0dot_reg[Dim], x[Dim], v[Dim];
	REAL a0_irr[Dim], a0dot_irr[Dim];
	REAL treg, mdot;
...
void Particle::calculateRegAccelerationFourthOrder(std::vector<Particle*> &particle) {
	...
	REAL dt, r, r2, vx, mdot, epsilon = 1e-6;
	REAL a0_reg[Dim], a0dot_reg[Dim], x[Dim], v[Dim];
	REAL a0_irr[Dim], a0dot_irr[Dim];
	REAL a2, a3, da_dt2, adot_dt, dt2, dt3, dt4;
