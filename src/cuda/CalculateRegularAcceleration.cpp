...
void CalculateRegAccelerationOnGPU(std::vector<int> IndexList, std::vector<Particle*> &particle){
	...
	AccRegReceive    = new double[ListSize][Dim]{0};
	AccRegDotReceive = new double[ListSize][Dim]{0};
	AccIrr           = new double[ListSize][Dim]{0};
	AccIrrDot        = new double[ListSize][Dim]{0};
	...
	// includes prediction
		...
		for (int i=0; i<ListSize; i++) {
			...
			RadiusOfAC2Send[i] = ptcl->RadiusOfAC*ptcl->RadiusOfAC;
		...
		CalculateAccelerationOnDevice(&ListSize, PositionSend, VelocitySend, AccRegReceive, AccRegDotReceive,
				MdotSend, RadiusOfAC2Send, NumNeighborReceive, ACListReceive);
#ifdef time_trace
		...
		for (int i=0; i<ListSize; i++) {
			...
			********************************************************/
			...
			//std::cout <<  "(" << NumNeighborReceive[0][i] << ")" << std::endl;
