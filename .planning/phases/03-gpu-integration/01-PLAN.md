# Plan 01: Create ParticleDataGPU Device Container

## Goal

Create a GPU SoA container struct with device pointers for all fields needed in GPU force calculation.
...
### Task 1: Create particle_data_gpu.h

Create a new header file with the GPU device container struct.

**File**: `src/particle_data_gpu.h`
...
- [ ] File compiles: `nvcc -c src/particle_data_gpu.h -o /dev/null`
- [ ] All j-particle fields have device pointers (8 fields)
- [ ] All i-particle fields have device pointers (8 fields)
- [ ] Output arrays match current GPU struct
- [ ] Pinned host buffers for async transfer

## Dependencies

None — this is the first plan in Phase 3.

## Pitfalls Addressed

None directly. This plan sets up the structure for later pitfall mitigation.
