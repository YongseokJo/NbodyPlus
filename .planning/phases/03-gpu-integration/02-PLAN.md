# Plan 02: Implement GPU Memory Management and Transfers

## Goal

Implement device memory allocation/deallocation and host-device transfer functions for the SoA GPU container.

## Requirements

- GPU-01: ParticleDataGPU container (memory management)
- GPU-02: Host-to-device transfer
...
### Task 1: Create particle_data_gpu.cu

**File**: `src/particle_data_gpu.cu`
...
### Task 2: Add to Makefile

Add `particle_data_gpu.cu` to the CUDA sources in the Makefile.

## Verification

- [ ] `make particle_data_gpu.o` compiles without errors
- [ ] All 16 particle field allocations succeed
- [ ] All 6 output array allocations succeed
- [ ] Pinned host buffers allocated
- [ ] Transfer functions compile
...
- **#4 GPU Transfer Fragmentation**: Uses `cudaMemcpyAsync` with streams for overlapping transfers
- **#8 Mixed Precision**: Explicit `#ifdef CUDA_FLOAT` paths for float/double conversion
